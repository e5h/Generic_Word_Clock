...
static STATUS_E i2c_write(UINT8 reg_addr_u8, UINT8* buf_u8, UINT16 len_u16)
{
    ...
    esp_err_t err = i2c_manager_write(rtc_s.i2c_port_e, rtc_s.address_u8, reg_addr_u8, buf_u8, len_u16);
...
static STATUS_E i2c_read(UINT8 reg_addr_u8, UINT8* buf_u8, INT16 len_u16)
{
    ...
    if(ESP_OK != i2c_manager_read(rtc_s.i2c_port_e, rtc_s.address_u8, reg_addr_u8, buf_u8, len_u16))
    {
        return STATUS_ERR;
    }
...
static STATUS_E reg_set(UINT8 reg_addr_u8, UINT8 mask_u8, UINT8 bits_u8)
{
    ...
    UINT8 reg_read_value_u8;
    UINT8 reg_write_value_u8;
    ...
    reg_write_value_u8 = reg_read_value_u8 & ((0xFF & ~mask_u8) | bits_u8);

    // Write the new register value
    status_e &= i2c_write(reg_addr_u8, &reg_write_value_u8, 1);
...
STATUS_E RTC_set_time(struct tm* p_timestamp_s)
{
    ...
    for(INT32 i = 0; i < sizeof(buffer_u8); i++)
    {
        status_e &= dec_to_bcd(&time_values_u8[i], &buffer_u8[i]);
    }

    if(status_e >= STATUS_OK)
    {
        status_e = i2c_write(RTC_REG_ADDR_SECONDS, buffer_u8, sizeof(buffer_u8));
    }
